...
| 层级 | 适用范围 | 默认状态 | 预览内容 |
|---|---|---|---|
| 通用规则 | 白名单未覆盖的命令 | **隐藏**（按 `Ctrl+/` 显示） | 目录→eza 树形，文件→bat 高亮，flags/子命令→补全描述 |
| 白名单覆盖 | `cd`、`ls`、`cat`、`vim`、`cp`、`mv`、`rm` 等 | **可见** | 目录→eza 树形，文件→bat 高亮 |

此外，`kill`（进程状态）、`systemctl`（服务状态）、环境变量（变量值）已有专属预览规则（默认可见）。

> [!TIP]
> **快捷键**：在 fzf-tab 补全界面中按 **`Ctrl+/`** 可随时切换预览面板的显示/隐藏。白名单命令可用此键隐藏预览以获得更多候选项空间，非白名单命令可用此键呼出预览查看文件内容或补全描述。
